...
public abstract class EntityThrowableCopy extends ProjectileEntity {
	...
	protected void onBlockHit(BlockRayTraceResult rayTraceResult) {
		// BlockState blockState = this.world.getBlockState(rayTraceResult.getPos());
		// blockState.onProjectileCollision(this.world, blockState, rayTraceResult, this);
	...
	public void tick() {
		if (isHit()){
			...
			if (System.currentTimeMillis() - hitTime > getMaxTimeAfterHit()){
				this.remove();
				return;
			}
			this.tickAfterHit();

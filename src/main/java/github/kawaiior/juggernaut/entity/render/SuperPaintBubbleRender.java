...
public class SuperPaintBubbleRender extends EntityRenderer<PaintBubbleEntity> {

    public static final ResourceLocation MAGIC = new ResourceLocation(Juggernaut.MOD_ID, "textures/entity/magic.png");

    public SuperPaintBubbleRender(EntityRendererManager renderManager) {
        super(renderManager);
    }
    ...
    public void render(PaintBubbleEntity entity, float entityYaw, float partialTicks, MatrixStack matrixStack, IRenderTypeBuffer buffer, int packedLight) {
        if (entity.isHit()) {
            ...
            RenderUtil.renderImageOnEntityRender(buffer, mat, RenderUtil.magicRender, 14F, 216/255F, 123/255F, 32/255F, 1F);
            matrixStack.pop();

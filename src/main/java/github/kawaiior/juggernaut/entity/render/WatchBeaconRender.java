...
public class WatchBeaconRender extends EntityRenderer<WatchBeaconEntity> {
    public static final ResourceLocation MAGIC = new ResourceLocation(Juggernaut.MOD_ID, "textures/entity/magic.png");

    public WatchBeaconRender(EntityRendererManager manager) {
        super(manager);
    }

    @Override
    public void render(WatchBeaconEntity entity, float entityYaw, float partialTicks,
                       MatrixStack matrixStack, IRenderTypeBuffer buffer, int packedLight) {
        matrixStack.push();
        Matrix4f mat = matrixStack.getLast().getMatrix();
        RenderUtil.renderImageOnEntityRender(buffer, mat, RenderUtil.magicRender, 4F, 0, 132/255F, 1, 1F);
        matrixStack.pop();
        super.render(entity, entityYaw, partialTicks, matrixStack, buffer, packedLight);
    }

    @Override
    public ResourceLocation getEntityTexture(WatchBeaconEntity reviveBeaconEntity) {
        return MAGIC;
    }
}
